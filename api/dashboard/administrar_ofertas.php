...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'create':
            ...
            if (!$oferta->setEstado('Activo')) {
                $result['exception'] = 'Estado incorrecto';
            } elseif (!is_uploaded_file($_FILES['archivo']['tmp_name'])) {
                $result['exception'] = 'Seleccione una imagen';
            } elseif (!$oferta->setImagen($_FILES['archivo'])) {
                $result['exception'] = $oferta->getFileError();
            } elseif ($oferta->saveFile($_FILES['archivo'], $oferta->getRuta(), $oferta->getImagen())) {
                $result['status'] = 1;
                $result['message'] = 'Oferta registrada correctamente';
            } else {
                $result['exception'] = Database::getException();
            }
            break;
