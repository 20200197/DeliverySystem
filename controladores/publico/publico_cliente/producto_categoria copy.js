...
function searchProductosPrecio() {
    ...
    fetch(API_PRODUCTO + 'searchProductoPrecio', {
        method: 'post',
        body: parameter
    }).then(function (request) {
        // Se verifica si la petición es correcta, de lo contrario se muestra un mensaje en la consola indicando el problema.
...
function searchProductosCalidad() {
    ...
    let params = new URLSearchParams(location.search);
    // Se obtienen los datos localizados por medio de las variables.
    const IDC = params.get("id_categoria");
    const NAMEC = params.get("nombre");
    parameter.append("id_categoria", IDC);

    // Se evita recargar la página web después de enviar el formulario.
    event.preventDefault();
    // Petición para obtener los datos del registro solicitado.
    fetch(API_PRODUCTO + 'searchProductoCalidadCategoria', {
        method: 'post',
        body: parameter
    }).then(function (request) {
        // Se verifica si la petición es correcta, de lo contrario se muestra un mensaje en la consola indicando el problema.
        if (request.ok) {
            ...
            request.json().then(function (response) {
                ...
                if (response.status) {
                    ...
                    readProductosCategoria(IDC, NAMEC);
                    readCategoria();
                } else {
